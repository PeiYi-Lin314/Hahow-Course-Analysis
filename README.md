...
使用 `Selenium` 搭配 `BeautifulSoup` 擷取 Hahow 各分類課程資料，並儲存為 `CSV` 檔。

📂 每頁課程資料將自動儲存至 `scraped_data/` 資料夾。
...
|---------------|------|
| `text`        | 課程名稱 |
| `link_tag`    | 課程連結，來自 `item.find("a")` |
| `popular_tag` | 熱門標籤，來自 `item.find("div", class_="sc-1x67v6w-0 sc-1x67v6w-1 jYbAhu icUvip")` |
</pre>
---
...
3. 可於程式中依需求調整以下變數：
   ...
   - `user_agent_list` / `proxy_list`：自訂並隨機使用 User-Agent 和 Proxy，模擬正常用戶行為以避免被封鎖 (還沒搞)
...
- 自動更新課程資訊（定期排程）  
- 加入其他欄位（如開課狀態、課程章節數等）  
- 整合 Power BI 或 Tableau 進行視覺化（目前已有相關作法，尚待測試 Jupyter 執行狀況）  

---

## 🙋‍♂️ 聯絡方式

若你對這個專案有興趣或有任何建議，歡迎聯絡我：  
